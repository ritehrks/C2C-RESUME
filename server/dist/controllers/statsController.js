...
exports.statsController = {
    ...
    getOverview: async (req, res) => {
        try {
            ...
            const [totalUsers, totalResumes, totalAnalyses] = await Promise.all([
                ...
                Resume_js_1.Resume.countDocuments(),
                AnalysisReport_js_1.AnalysisReport.countDocuments(),
            ]);
            // Users registered today
            const today = new Date();
            today.setHours(0, 0, 0, 0);
            const usersToday = await User_js_1.User.countDocuments({ createdAt: { $gte: today } });
            // Resumes created this week
            const weekAgo = new Date();
            weekAgo.setDate(weekAgo.getDate() - 7);
            const resumesThisWeek = await Resume_js_1.Resume.countDocuments({ createdAt: { $gte: weekAgo } });
            // Deep analyses used today
            const deepAnalysesToday = await AnalysisReport_js_1.AnalysisReport.countDocuments({
                analysisType: 'deep',
                createdAt: { $gte: today },
            });
            res.json({
    ...
    getResumeStats: async (req, res) => {
        try {
            ...
            const templateStats = await Resume_js_1.Resume.aggregate([
                {
                    $group: {
                        _id: '$templateId',
                        count: { $sum: 1 },
            ...
            // Recent resumes
            const recentResumes = await Resume_js_1.Resume.find()
                .sort({ createdAt: -1 })
                .limit(5)
    ...
    getAnalysisStats: async (req, res) => {
        try {
            ...
            const roleStats = await AnalysisReport_js_1.AnalysisReport.aggregate([
                {
                    $group: {
                        _id: '$role',
                        count: { $sum: 1 },
            ...
            const analysisTypeStats = await AnalysisReport_js_1.AnalysisReport.aggregate([
                {
                    $group: {
                        _id: '$analysisType',
                        count: { $sum: 1 },
            ...
            analysisTypeStats.forEach((item) => {
    ...
    getRecentActivity: async (req, res) => {
        try {
            ...
            // Get recent users
            const recentUsers = await User_js_1.User.find()
                .sort({ createdAt: -1 })
                .limit(5)
                .select('name email createdAt role');
            // Get recent analyses
            const recentAnalyses = await AnalysisReport_js_1.AnalysisReport.find()
                .sort({ createdAt: -1 })
                .limit(5)
                .populate('userId', 'name email')
                .select('role analysisType results.overallScore createdAt');
            // Get all users for the table
            const allUsers = await User_js_1.User.find()
                .sort({ createdAt: -1 })
                .limit(50)
    ...
    async getUserList(req, res) {
        try {
            ...
            const usersWithCounts = await Promise.all(users.map(async (user) => {
                const analysisCount = await AnalysisReport_js_1.AnalysisReport.countDocuments({ userId: user._id });
                return {
                    ...user,
                    analysisCount,
