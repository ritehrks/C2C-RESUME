...
async function runSimpleAnalysis(resumeText, selectedRole) {
    ...
    // 2. KEYWORD MATCHING
    const matchedKeywords = roleProfile.keywords.filter(k => resumeLower.includes(k.toLowerCase()));
    const missingKeywords = roleProfile.keywords.filter(k => !resumeLower.includes(k.toLowerCase()));
